...
cat bamlists/197.txt  | while read line; do samtools depth $line | awk '{sum+=$3} END { print sum/NR}' >> bamlists/197.cov; done;
```
...
```{r, warning=FALSE, message=FALSE}
filtered<- top_frac(samples, .90, Coverage)
...
save(filteredbams, file="metadata/filtered.rda")
write(filteredbams$String, file="bamlists/177.txt")
write(filteredbams$String, file="bamlists/177.bamlist")
```