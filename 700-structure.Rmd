...
qdata<-read_delim("outputs/700/results_job_T1_q", col_names = FALSE, delim=" ")
qdata$Individual<-gsub("\\s+", "", qdata$X1)
qdata$Population<-gsub("\\s+", "", qdata$X2)
qdata<-qdata %>% rename(Q1=X3, Q2=X4) %>% select(Population, Individual, Q1, Q2)

pops<-read_tsv("outputs/700/popList.txt", col_names = FALSE)
qdata$Pop<-pops$X1

df<-qdata %>% select(Population, Pop, Individual, Q1, Q2) %>% gather(key=Ancestry, value=Q, 4:5)
